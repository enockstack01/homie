...
export default async function SuperAdminUsageLogsPage() {
  let logs: UsageLog[];
  try {
    logs = await callBackend<UsageLog[]>("/v1/super-admin/usage-logs");
  } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <p className="text-sm text-foreground/50">Platform-wide consumption - most recent 100 shown.</p>

      <CollapsibleSection title="Log entries" count={logs.length}>
        <UsageLogsTable logs={logs} userColumnLabel="User" showOrgMarginColumn />
      </CollapsibleSection>
    </div>
  );
}
